...
class administration(commands.Cog):
    ...
    async def _mute(self, ctx, member:discord.Member, duration, *, reason=None):
        ...
        progress = await ctx.send('Пользователь замучен!')
        try:
    ...
    async def report(self, ctx, member: discord.Member=None, *, reason=None):
        if not collection.find_one({"guild_id": ctx.guild.id}):
            embed = discord.Embed(title="Ошибка", description="Система жалоб на сервере не включена!\nЧтобы включить пропишите - `>report-channel <on/off> <channel>`", color=discord.Color.red())
            await ctx.send(embed=embed)
        else:
            if member is None:
